...
function incidentReport(incidentId) { const state=hydrateIncident(incidentId); if(!state) throw new Error('Incident not found'); const leading=state.hypotheses[0]; const cited=state.findings.flatMap(f=>f.evidence_ids_json||[]); return `# ${state.incident.id}: ${state.incident.title}\n\n## Executive summary\nTraceMind investigated this ${state.incident.severity} incident with ${state.tasks.length} specialist tasks. The leading explanation is **${leading?.title||'under investigation'}** (${leading?Math.round(leading.confidence*100):0}% confidence).\n\n## Timeline\n${state.timeline.map(e=>`- ${e.occurred_at} — ${e.title}: ${e.detail}`).join('\n')}\n\n## Evidence-backed findings\n${state.findings.map(f=>`- [${f.certainty.toUpperCase()}] ${f.statement} (${(f.evidence_ids_json||[]).join(', ')||'no direct reference'})`).join('\n')}\n\n## Competing hypotheses\n${state.hypotheses.slice(1).map(h=>`- ${h.title}: ${h.status} (${Math.round(h.confidence*100)}%)`).join('\n')||'- None recorded'}\n\n## Actions and approvals\n${state.actions.map(a=>`- ${a.kind} → ${a.status}`).join('\n')||'- No actions proposed'}\n\n## Evidence references\n${[...new Set(cited)].join(', ')||'No evidence references available yet.'}\n`; }
...
const server=createServer(async(req,res)=>{ try { const url=new URL(req.url,`http://${req.headers.host}`); const send=(status,data)=>{res.writeHead(status,{'Content-Type':'application/json'});res.end(json(data));};
  ...
  const report=url.pathname.match(/^\/api\/incidents\/([^/]+)\/report$/); if(req.method==='GET'&&report){res.writeHead(200,{'Content-Type':'text/markdown; charset=utf-8'});return res.end(incidentReport(report[1]));}
  const audit=url.pathname.match(/^\/api\/incidents\/([^/]+)\/audit$/); if(req.method==='GET'&&audit){const state=hydrateIncident(audit[1]);return send(200,{incidentId:audit[1],policy:{read_tools:['query_metrics','search_logs','get_deployments','get_git_diff','get_database_health','get_infrastructure_events'],approval_required:['rollback_deployment','create_github_issue']},approvals:rows('SELECT * FROM approvals WHERE incident_id=?',audit[1]),actions:state?.actions||[]});}

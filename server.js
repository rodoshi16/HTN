...
async function investigate(incidentId, scenario) {
  ...
  const independent=await independentModel(evs.map(e=>({id:e.id,source:e.source,payload:e.payload}))); message(incidentId,'model_review','incident_agent','model_review',`Primary provider: ${primary.provider}; independent reviewer: ${independent.provider}.`);
...
export async function approve(incidentId, actionId) { const action=row('SELECT * FROM actions WHERE id=? AND incident_id=?',actionId,incidentId); if(!action||action.status!=='pending_approval') throw new Error('Approval is not pending'); db.prepare('UPDATE actions SET status=?,executed_at=? WHERE id=?').run('executed_simulation',now(),actionId); db.prepare('UPDATE approvals SET status=?,decided_at=?,decided_by=? WHERE action_id=?').run('approved',now(),'engineer',actionId); message(incidentId,'safety_agent','deployment_agent','approval','Human approval recorded; executing only in simulated environment.'); await sleep(100); finding(incidentId,'deployment_agent','Simulated rollback completed. API 500 rate returned toward baseline.','fact',.92,[]); db.prepare('UPDATE incidents SET status=?,updated_at=? WHERE id=?').run('mitigated',now(),incidentId); emit(incidentId,'complete',{incidentId,status:'mitigated'}); }
